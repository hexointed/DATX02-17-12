\chapter{Implementation}
	...
	\section{Reference Shader} \label{implshader}
		...
		\subsection{Orthogonal culling}
			...
			field of view (Frustum culling) 
			
			// but we can't do that because of
			//GLSL does not allow us to run code on the CPU. 
			// öhm inte glsl kanske men ville vi implementera det går det ju lätt i .jsx
			
			Even if we could use frustum culling it would probably not be enough, each ray 
			should only care for the objects that it might hit, not all 
		...
		\subsection{Assembler Based}
			...
			initially a single thread. This thread will create a render thread,
			which will calculate the color of the next pixel, and one new version of
			itself. Lastly it simply terminates it's execution, freeing up the core
			to request a new thread. The new version of the thread keeps track of
			the next pixel to be rendered. This will render all pixels as
			concurrently as the GPU has cores, minimizing core stalls and therefore
			maximizing GPU usage. //Beskriv minimum implementation av sphere
			tracing.
