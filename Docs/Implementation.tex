\chapter{Implementation}
	...
	\section{Reference Shader} \label{implshader}
		...
		\subsection{Orthogonal Culling}
			...
			culling) but we can't do that because of GLSL does not allow us to
			run code on the CPU. Even if we could use frustum culling it would
			...
			scene for ray and calculating which it can hit. 
			...
			the directional line of the ray, its the line's closest point to the
			object is known. Then the distance between the line and the sphere can be
		...
		\subsection{Assembler Based}
			...
			language wasn't completed until later in the project. Therefore the 
