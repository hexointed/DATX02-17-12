\chapter{Discussion}
	...
	\section{Results}
		...
		\subsection{Software Shader}
			...
			Infinite planes require a lot of computing power to render. When
			the camera is oriented so that the field of view is along an infinite
			plane some rays will travel parallell to the plane, they then march 
			...
			to render the horizon and might also increase performance, with the
			drawback that deformations to a plane would be harder to describe 
			mathematically. This way to render planes has not been tested and
			the potential results are merely speculation.
